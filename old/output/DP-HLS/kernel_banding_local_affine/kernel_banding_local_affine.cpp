...
void BandingLocalAffine::PE::Compute(char_t local_query_val,
...
void BandingLocalAffine::InitializeScores(
...
void BandingLocalAffine::UpdatePEMaximum(
...
void BandingLocalAffine::InitializeMaxScores(ScorePack (&max)[PE_NUM], idx_t qry_len, idx_t ref_len)
{
...
void BandingLocalAffine::Traceback::StateMapping(tbp_t tbp, TB_STATE &state, tbr_t &navigation)
{
...
void BandingLocalAffine::Traceback::StateInit(tbp_t tbp, TB_STATE &state)
{
    if (tbp(1, 0) == TB_DIAG)
    {
        state = TB_STATE::MM;
