...
void OverlapLinearPrefixSuffix::InitializeScores(
...
void OverlapLinearPrefixSuffix::PE::Compute(char_t local_query_val,
...
void OverlapLinearPrefixSuffix::UpdatePEMaximum(
...
void OverlapLinearPrefixSuffix::InitializeMaxScores(ScorePack (&max)[PE_NUM], idx_t qry_len, idx_t ref_len){
    for (int i = 0; i < PE_NUM; i++)
    {
#pragma HLS unroll
...
void OverlapLinearPrefixSuffix::Traceback::StateMapping(tbp_t tbp, TB_STATE &state, tbr_t &navigation){
