...
void output_result_array_to_stream (complex<float> data_6[FFT_NUM], ac_channel<vec_wrapper<complex<float>, UF*2>> & dataOut
){
    PostP_Fwd_loop: for (int i = 0; i < FFT_NUM/(UF*2); i++)  { 
        #pragma hls_pipeline_init_interval 1
        vec_wrapper<complex<float>, UF*2> temp;
...
void reverse_input_stream_UF4 (
    ac_channel<vec_wrapper<complex<float>, UF*2>> & dataIn,
    ac_channel<vec_wrapper<complex<float>, UF*2>> & reverse_in_stream_vector
){
...
template<int stage> void FFT_stage_spatial_unroll(complex<float> data_ld[FFT_NUM], complex<float> data_st[FFT_NUM]){
    ...
    if(bflyStep < UF){
        ...
        for (uint16_t m = 0; m < FFT_NUM; m += bflySize) {
            #pragma hls_unroll yes factor=UF>>(stage-1)
            #pragma hls_pipeline_init_interval 1
            L_Group_loop: 
            ...
            for (uint16_t k = 0; k < bflyStep; ++k) {
                #pragma hls_pipeline_init_interval 1
                #pragma hls_unroll yes factor=UF
                // Butterfly computations
                uint16_t index = indexMultiply * k;
                auto tw = twiddles[index];   
                uint16_t i0 = 0 + k; 
...
void FFT_Stage1_vectorstream_parameterize(
    ac_channel<vec_wrapper<complex<float>, UF*2>> & reverse_in_stream_vector,
    ac_channel<vec_wrapper<complex<float>, UF*2>> & data_s1_stream_vector
){
    FFT_Stage1: for (int m = 0; m < FFT_NUM/(2*UF); m += 1) {
        #pragma hls_pipeline_init_interval 1
        auto tw = complex<float>(0,0); 
        vec_wrapper<complex<float>, UF*2> data = reverse_in_stream_vector.read();
        vec_wrapper<complex<float>, UF*2> data_out;
        for (int i = 0; i < UF; i++){
            auto data0 = data[i*2];
...
FFT_Stage2:  for (int m = 0; m < FFT_NUM; m += 2*UF) {
        #pragma hls_pipeline_init_interval 1
        vec_wrapper<complex<float>, UF*2> data = data_s1_stream_vector.read();
...
void FFT_DIT_spatial_unroll_CY_stream_vector( 
    ac_channel<vec_wrapper<complex<float>, UF*2>> & dataIn,  
    ac_channel<vec_wrapper<complex<float>, UF*2>> & dataOut
){
