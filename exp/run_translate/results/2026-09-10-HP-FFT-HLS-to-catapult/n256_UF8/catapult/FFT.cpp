...
void output_result_array_to_stream (complex<float> data_6[FFT_NUM], ac_channel<hls_vector<complex<float>, UF*2>> & dataOut
){
    PostP_Fwd_loop: for (int i = 0; i < FFT_NUM/(UF*2); i++)  { 
        #pragma hls_pipeline_init_interval 1
        hls_vector<complex<float>, UF*2> temp;
...
void reverse_input_stream_UF8 (
    ac_channel<hls_vector<complex<float>, UF*2>> & dataIn,
    ac_channel<hls_vector<complex<float>, UF*2>> & reverse_in_stream_vector
){
    ...
    #pragma hls_pipeline_init_interval FFT_NUM/(2*UF)
...
template<int stage> void FFT_stage_spatial_unroll(complex<float> data_ld[FFT_NUM], complex<float> data_st[FFT_NUM]){
    ...
    if(bflyStep < UF){
        ...
        for (uint16_t m = 0; m < FFT_NUM; m += bflySize) {
            #pragma hls_pipeline_init_interval 1
            #pragma hls_unroll factor=UF>>(stage-1)
            L_Group_loop: 
...
void FFT_Stage1_vectorstream_parameterize(
    ac_channel<hls_vector<complex<float>, UF*2>> & reverse_in_stream_vector,
    ac_channel<hls_vector<complex<float>, UF*2>> & data_s1_stream_vector
){
    // #pragma hls_design block (dataflow inline not expressible in Catapult as a region)
    FFT_Stage1: for (int m = 0; m < FFT_NUM/(2*UF); m += 1) {
        #pragma hls_pipeline_init_interval 1
        // Butterfly computations
        auto tw = complex<float>(0,0); 
        hls_vector<complex<float>, UF*2> data = reverse_in_stream_vector.read();
        hls_vector<complex<float>, UF*2> data_out;
        for (int i = 0; i < UF; i++){
            auto data0 = data[i*2];
...
FFT_Stage2:  for (int m = 0; m < FFT_NUM; m += 2*UF) {
        #pragma hls_pipeline_init_interval 1
        hls_vector<complex<float>, UF*2> data = data_s1_stream_vector.read();
...
void FFT_DIT_spatial_unroll_CY_stream_vector( 
    ac_channel<hls_vector<complex<float>, UF*2>> & dataIn,  
    ac_channel<hls_vector<complex<float>, UF*2>> & dataOut
){

    // Dataflow region translated to per-function block directives below
