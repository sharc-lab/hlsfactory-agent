# HP-FFT-HLS: n1024_UF1 (Catapult HLS Port)

## Description
This is an HLS implementation of an FFT (Fast Fourier Transform) design for FFT size N=1024 (EXP2_FFT=10) with unrolling factor UF=1.

FFT with spatial unrolling factor UF=1 and stream-based vector interface. Uses ac_channel<vect<complex<float>, UF*2>> for input/output, bit-reversal via reverse_input_stream_UF1 (UF1), and template-based FFT_stage_spatial_unroll for each stage. Stage 1 is handled specially via FFT_Stage1_vectorstream_parameterize.

## Top-Level Function
`FFT_TOP`

## Interface
Channel-based (ac_channel<vect<complex<float>, UF*2>>)
