...
void output_result_array_to_stream (complex<float> data_6[FFT_NUM], ac_channel<vect<complex<float>, UF*2>> & dataOut
){
    PostP_Fwd_loop: for (int i = 0; i < FFT_NUM/(UF*2); i++)  { 
        #pragma hls_pipeline_init_interval 1
        vect<complex<float>, UF*2> temp;
...
void reverse_input_stream_UF1 (
    ac_channel<vect<complex<float>, UF*2>> & dataIn,
    ac_channel<vect<complex<float>, UF*2>> & reverse_in_stream_vector
){
...
void FFT_Stage1_vectorstream_parameterize(
    ...
    FFT_Stage1: for (int m = 0; m < FFT_NUM; m += 2 * UF) {
        ...
        vect<complex<float>, UF*2> data = reverse_in_stream_vector.read();
...
void FFT_DIT_spatial_unroll_CY_stream_vector( 
    ac_channel<vect<complex<float>, UF*2>> & dataIn,  
    ac_channel<vect<complex<float>, UF*2>> & dataOut
){
