...
void output_result_array_to_stream (complex<float> data_6[FFT_NUM], ac_channel<vec_cfloat> & dataOut
){
...
void reverse_write_stream_output (
    complex<float> data_in_cyclic[UF*2][FFT_NUM/(UF*2)],
    ac_channel<vec_cfloat> & reverse_in_stream_vector
) {
    const int TIME_STEP =  FFT_NUM/(UF*2);
    const int PAR =  UF*2;
...
void FFT_Stage1_vectorstream_parameterize(
    ac_channel<vec_cfloat> & reverse_in_stream_vector,
    ac_channel<vec_cfloat> & data_s1_stream_vector
){
    // #pragma HLS performance target_ti=FFT_NUM/(2*UF) unit=cycle
...
#pragma hls_pipeline_init_interval 1
        ...
        vec_cfloat data = reverse_in_stream_vector.read();
        vec_cfloat data_out;
        for (int i = 0; i < UF; i++){
            auto data0 = data[i*2];
...
#pragma hls_pipeline_init_interval 1
        vec_cfloat data = data_s1_stream_vector.read();
