# HP-FFT-HLS: n1024_UF8

## Description
This is an HLS implementation of an FFT (Fast Fourier Transform) design for FFT size N=1024 (EXP2_FFT=10) with unrolling factor UF=8.

FFT with spatial unrolling factor UF=8 and stream-based vector interface. Uses hls::stream<hls::vector<complex<float>, UF*2>> for input/output, bit-reversal via reverse_input_stream_UF1 (UF1) or reverse_input_stream_UF2 (UF2+), and template-based FFT_stage_spatial_unroll for each stage. Stage 1 is handled specially via FFT_Stage1_vectorstream_parameterize.
