...
void output_result_array_to_stream (complex<float> data_6[FFT_NUM], ac_channel<vec16_cfloat> & dataOut
){
...
void reverse_input_stream_UF8 (
    ac_channel<vec16_cfloat> & dataIn,
    ac_channel<vec16_cfloat> & reverse_in_stream_vector
){
...
void FFT_Stage1_vectorstream_parameterize(
    ac_channel<vec16_cfloat> & reverse_in_stream_vector,
    ac_channel<vec16_cfloat> & data_s1_stream_vector
){
    #pragma hls_design block
    FFT_Stage1: for (int m = 0; m < FFT_NUM/(2*UF); m += 1) {
        #pragma hls_pipeline_init_interval 1
        auto tw = complex<float>(0,0); 
        vec16_cfloat data = reverse_in_stream_vector.read();
        vec16_cfloat data_out;
        for (int i = 0; i < UF; i++){
            auto data0 = data[i*2];
...
FFT_Stage2:  for (int m = 0; m < FFT_NUM; m += 2*UF) {
        #pragma hls_pipeline_init_interval 1
        vec16_cfloat data = data_s1_stream_vector.read();
...
void FFT_DIT_spatial_unroll_CY_stream_vector( 
    ac_channel<vec16_cfloat> & dataIn,  
    ac_channel<vec16_cfloat> & dataOut
){
