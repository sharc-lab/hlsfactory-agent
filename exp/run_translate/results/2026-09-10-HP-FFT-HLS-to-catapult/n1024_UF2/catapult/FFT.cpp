...
void output_result_array_to_stream (complex<float> data_6[FFT_NUM], ac_channel<vec<complex<float>, UF*2>> & dataOut
){
    PostP_Fwd_loop: for (int i = 0; i < FFT_NUM/(UF*2); i++)  { 
        #pragma hls_pipeline_init_interval 1
        vec<complex<float>, UF*2> temp;
...
void reverse_read_stream_input (
    ac_channel<vec<complex<float>, UF*2>> & dataIn,
    ...
    const int PAR =  UF*2;
    // Dropped: #pragma inline off (Vitis-specific, no Catapult equivalent)
...
void reverse_write_stream_output (
    complex<float> data_in_cyclic[UF*2][FFT_NUM/(UF*2)],
    ac_channel<vec<complex<float>, UF*2>> & reverse_in_stream_vector
) {
    const int TIME_STEP =  FFT_NUM/(UF*2);
    const int PAR =  UF*2;
    // Dropped: #pragma inline off (Vitis-specific, no Catapult equivalent)
...
void FFT_Stage1_vectorstream_parameterize(
    ac_channel<vec<complex<float>, UF*2>> & reverse_in_stream_vector,
    ac_channel<vec<complex<float>, UF*2>> & data_s1_stream_vector
){
    ...
    FFT_Stage1: for (int m = 0; m < FFT_NUM/(2*UF); m += 1) {
        ...
        vec<complex<float>, UF*2> data = reverse_in_stream_vector.read();
        vec<complex<float>, UF*2> data_out;
        for (int i = 0; i < UF; i++){
            auto data0 = data[i*2];
...
FFT_Stage2:  for (int m = 0; m < FFT_NUM; m += 2*UF) {
        #pragma hls_pipeline_init_interval 1
        vec<complex<float>, UF*2> data = data_s1_stream_vector.read();
...
void FFT_DIT_spatial_unroll_CY_stream_vector( 
    ac_channel<vec<complex<float>, UF*2>> & dataIn,  
    ac_channel<vec<complex<float>, UF*2>> & dataOut
){
...
void FFT_TOP(ac_channel<vec<complex<float>, UF*2>> & in, 
    ac_channel<vec<complex<float>, UF*2>> & out
){
