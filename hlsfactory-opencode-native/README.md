...
Unlike the original `hlsfactory-opencode` which uses Python to call OpenCode for each stage, this version runs OpenCode directly with a single comprehensive prompt. OpenCode handles the entire pipeline autonomously.
...
## Advantages Over Python Orchestration

- **Single Context**: The agent maintains full context throughout the pipeline
- **Smarter Decisions**: Can adapt based on what it learns during analysis
- **Simpler Setup**: No Python dependencies required
- **Better Error Recovery**: Can intelligently handle cross-stage issues
