...
//            for( int unrolled = 0; unrolled < rr_unroll_factor; unrolled++ )
            {
                ...
                if( outer == 0 )
                {
                    const ap_uint<xyz_derived_pt::bit_width>& xyz_derived_uint = xyz_derived_stream_in.read();
                    const ap_uint<rys_wt_pt::bit_width>& wt_rys_uint = rys_wt_stream_in.read();
                    const ap_uint<tb_b_pt::bit_width>& tb_cci_uint = tb_b_stream_in.read();
                    const ap_uint<tb_c_pt::bit_width>& tb_ccd_uint = tb_c_stream_in.read();

                    xyz_derived = xyz_derived_uint;
                    wt_rys = wt_rys_uint;
                    tb_cci = tb_cci_uint;
                    tb_ccd = tb_ccd_uint;

                    xyz_derived_stream_out.write( xyz_derived_uint );
                    rys_wt_stream_out.write( wt_rys_uint );
                    tb_b_stream_out.write( tb_cci_uint );
                    tb_c_stream_out.write( tb_ccd_uint );
                }
...
#pragma hls unroll
                    ...
                    ucba_stream_out_0.write(packed_dcbaox);
                }
            }
        }
