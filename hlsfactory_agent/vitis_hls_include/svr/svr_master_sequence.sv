...
`ifndef SVR_MASTER_SEQUENCE__SV
    ...
    class svr_master_sequence#(int DATA_WIDTH = 32) extends svr_base_sequence#(DATA_WIDTH);
        ...
        virtual task body();
            ...
            total_TestVector_num = get_TestVector_num();
            fork
                begin
                    @finish;
            ...
            fork
                while (get_TestVector_num() > 0) begin
                    case( cfg.prt_type )
                        AP_VLD, AP_NONE, AP_FIFO, AXIS: begin
                            sema_ready.get();
                            `uvm_info(this.get_full_name(), $sformatf("get ap_ready"), UVM_MEDIUM)
                        end
                    endcase
                    pop_TestVector();
                ...
                for (int i = 0; i < total_TestVector_num; i++) begin
                    case( cfg.prt_type )
                        AP_VLD, AP_NONE, AP_FIFO, AXIS: begin
