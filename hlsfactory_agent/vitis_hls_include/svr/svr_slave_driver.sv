...
`ifndef SVR_SLAVE_DRIVER__SV
    ...
    task svr_slave_driver::run_phase (uvm_phase phase);
        ...
        if ((cfg.prt_type == AP_NONE)) begin //for ap_none, don't know when data is valid or ready
    ...
    task svr_slave_driver::get_and_drive(svr_transfer#(DATA_WIDTH) tr);  
        ...
        else if ((cfg.prt_type == AP_VLD) || (cfg.prt_type == AP_ACK)) begin    // no delay for AP-VLD / AP-ACK protocol
        end
        else
