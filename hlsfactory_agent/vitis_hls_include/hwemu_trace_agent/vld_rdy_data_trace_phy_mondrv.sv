    ...
    virtual function int get_lines(ref longint clkcyc[string][$], ref logic[DATAW-1:0] data[$], string evtf[$], int dataw[$]);
        ...
        while($fgets(line, fp)) begin
            ...
            foreach(evtf[i])  begin
                ...
                if(aa==-1&&i!=evtf.size-1) $fatal(1, "Error: file:%s format is wrong at %t. TB file:%s line:%d", FilePath, $time, `__FILE__, `__LINE__ );
            ...
            foreach(dataw[i]) begin
                ...
                shift+=dataw[i];
                aa = get_next_field(line);
                if(aa==-1&&i!=dataw.size-1) $fatal(1, "Error: file:%s format is wrong at %t. TB file:%s line:%d", FilePath, $time, `__FILE__, `__LINE__ );
                line = line.substr(aa, line.len-1);
            end
