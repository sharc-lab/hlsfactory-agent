...
#define _AXIS_CHANNEL_INIT(CHAN_NAME, INIT_VALUE)                             \
    if constexpr (has_##CHAN_NAME) CHAN_NAME = INIT_VALUE;

  axis(){

    _AXIS_CHANNEL_INIT(keep, -1)
    _AXIS_CHANNEL_INIT(strb, -1)
    _AXIS_CHANNEL_INIT(user, 0)
    _AXIS_CHANNEL_INIT(last, 0)
    _AXIS_CHANNEL_INIT(id,   0)
    _AXIS_CHANNEL_INIT(dest, 0)

  }

  axis(
    Type_data data_init,
    ap_uint<width_keep> keep_init,
    ap_uint<width_strb> strb_init,
    ap_uint<width_user> user_init,
    ap_uint<width_last> last_init,
    ap_uint<width_id> id_init,
    ap_uint<width_dest> dest_init
  ){

    _AXIS_CHANNEL_INIT(data, data_init)
    _AXIS_CHANNEL_INIT(keep, keep_init)
    _AXIS_CHANNEL_INIT(strb, strb_init)
    _AXIS_CHANNEL_INIT(user, user_init)
    _AXIS_CHANNEL_INIT(last, last_init)
    _AXIS_CHANNEL_INIT(id,   id_init)
    _AXIS_CHANNEL_INIT(dest, dest_init)

  }

#undef _AXIS_CHANNEL_INIT
...
// SLX: The specializations of hls::stream below use intrinsics that are not
// implemented yet. They are still API compatible with the default stream
// implementation. Additionally, not using this specialization in the HW means
// that HW and TB definitely use the same ABI on the toplevel interface.
#if defined(AESL_SYN) && !defined(__PRESYNTHESIS__)
