...
`ifndef FILE_READ_AGENT__SV
    ...
    class file_read_agent#(int DATA_WIDTH = 32) extends uvm_object;
        ...
        string TV_FILE_COMPARE = "";
        integer fp_compare;
        DATA_QUEUE_TYPE TV_Queue_compare[$];
        ...
        `uvm_field_int(is_binary, UVM_DEFAULT)

        `uvm_field_string(TV_FILE_COMPARE, UVM_DEFAULT)
        `uvm_field_int(fp_compare, UVM_DEFAULT)
        ...
        extern virtual       function void    read_TVIN_file_ascii();
        ...
        extern virtual       function void     set_binary(integer bin);
        extern virtual       function void    read_TVIN_file();
        extern virtual       function void    read_TVIN_file_bin();

        extern virtual       function void    set_compare_file_path(string path);
        extern virtual       function void    stable_single_input_compare();
        extern virtual       function void    stable_inout_compare();
        extern virtual       function string  extract_port_name(string path);
        
            
    endclass            

    function string file_read_agent::extract_port_name(string path);
        int path_len = path.len();
        string sub_str = "";
        for(int i = path_len-1; i >= 0; i=i-1)begin
            if(path.substr(i,i).compare("/") == 0) begin
                sub_str = path.substr(i+1, path_len-5);   
                break;
            end
        end
        path_len = sub_str.len();
        for(int i = 0; i < path_len; i=i+1)begin
            if(sub_str.substr(i, i+5).compare("autotv"))begin
                sub_str = sub_str.substr(i+6, path_len-1);
                break;
            end
        end
        path_len = sub_str.len();
        for(int i = 0; i < path_len; i=i+1)begin
            if(sub_str.substr(i,i).compare("_") == 0)begin
                sub_str = sub_str.substr(i+1, path_len-1);
                break;
            end
        end        
        return sub_str;
    ...
    function void file_read_agent::open_read_file();
        if (is_binary == 0) begin
            set_fp($fopen(TV_FILE, "r"));
        end
        else begin
            set_fp($fopen(TV_FILE, "rb"));
        end
    ...
    function void file_read_agent::read_TVIN_file_ascii();
    ...
    function void file_read_agent::set_binary(integer bin);
        is_binary = bin;
    endfunction

    function void file_read_agent::read_TVIN_file();
        if(is_binary == 0)
            read_TVIN_file_ascii();
        else
            read_TVIN_file_bin();
    endfunction   

    function void file_read_agent::read_TVIN_file_bin();
        reg [DATA_WIDTH-1:0] data;
        integer ret;
        DATA_QUEUE_TYPE TV;
        ...
        while(trans_depth != 64'h5a5aa5a50f0ff0f0) begin
            while(trans_depth != 0) begin
                ret = $fread(data, fp);
                if(ret == -1)
                    `uvm_fatal(this.get_full_name(), "ERROR: Simulation reading TVIN failed.");
                if(ret == 0)
                    `uvm_fatal(this.get_full_name(), "ERROR: Simulation reading TVIN got no input data.");
                TV.push_back(data);                
                trans_depth = trans_depth - 1;
                `uvm_info(this.get_full_name(), $sformatf("read data: %0d", data), UVM_DEBUG)
            end
            TV_Queue.push_back(TV);
            ret = $fread(trans_depth, fp);
    ...
    function void file_read_agent::set_compare_file_path(string path);
        TV_FILE_COMPARE = path;
        if(is_binary == 0)
            fp_compare = $fopen(TV_FILE_COMPARE, "r");
        else
            fp_compare = $fopen(TV_FILE_COMPARE, "rb");
    endfunction

    function void file_read_agent::stable_single_input_compare();
        DATA_QUEUE_TYPE reference;
        read_TVIN_file();        
        reference = TV_Queue[0];
        foreach(TV_Queue[i]) begin
            if(i == 0)  continue;
            if(TV_Queue[i] != reference) begin
                `uvm_fatal(this.get_full_name(), $sformatf("input %s cannot change between kernel calls by the testbench", extract_port_name(TV_FILE)));
            end
        end
    ...
    function void file_read_agent::stable_inout_compare();
        integer fp_original;
        read_TVIN_file();
        TV_Queue_compare = TV_Queue;
        fp_original = fp;
        fp = fp_compare;
        read_TVIN_file();
        
        for(int i = 1; i < TV_Queue_compare.size(); i++) begin
            if(TV_Queue[i-1] != TV_Queue_compare[i])begin
                `uvm_fatal(this.get_full_name(), $sformatf("inout argument %s cannot change between current transaction input and last transaction output", extract_port_name(TV_FILE_COMPARE)));
            end 
        end
