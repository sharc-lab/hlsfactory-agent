...
HLS_CONSTEXPR_EXTRA inline bool _AP_ctype_op_get_bit(_Tp1& var, const _Tp2& index) {
  return !!(var & (1ull << (index)));
}
template <typename _Tp1, typename _Tp2, typename _Tp3>
HLS_CONSTEXPR_EXTRA inline _Tp1 _AP_ctype_op_set_bit(_Tp1& var, const _Tp2& index, const _Tp3& x) {
  var |= (((x) ? 1ull : 0ull) << (index));
  return var;
}
template <typename _Tp1, typename _Tp2, typename _Tp3>
HLS_CONSTEXPR_EXTRA inline _Tp1 _AP_ctype_op_get_range(_Tp1& var, const _Tp2& low,
                                   const _Tp3& high) {
  _Tp1 r = var;
  ap_ulong mask = -1ll;
...
HLS_CONSTEXPR_EXTRA inline _Tp1 _AP_ctype_op_set_range(_Tp1& var, const _Tp2& low, const _Tp3& high,
                                   const _Tp4& x) {
  ap_ulong mask = -1ll;
...
struct ap_fixed_base : _AP_ROOT_TYPE<_AP_W, _AP_S> {
 ...
 private:
  ...
  HLS_CONSTEXPR_EXTRA AP_INLINE AP_NODEBUG void report() {
    ...
    static_assert(_AP_W <= MAX_MODE(AP_INT_MAX_W) * 1024, 
              "[E] ap_fixed<...>: Bitwidth exceeds the "
              "default max value. Please use macro "
...
#else
    // TODO: to be filled for synthesis
    return 0;
#endif
  }
 ...
 public:
  ...
  AP_INLINE AP_NODEBUG ap_fixed_base(const char* s, signed char rd = 0) {
    ...
    _AP_WARNING(str.empty(), "ap_fixed<> is being initialized with an empty C string, which can result in undefined behavior during simulation/synthesis"); 
...
#ifdef __PRESYNTHESIS__
#error @Unsupported_NON_C99STRING@
#endif
 ...
 */
  AP_WEAK AP_NODEBUG ap_fixed_base(double d) {
    ...
    ap_int_base<DOUBLE_EXP + 1, true> exp, bias = DOUBLE_BIAS;
    ap_int_base<DOUBLE_EXP, false> exp_tmp;
    exp_tmp.V = _AP_ROOT_op_get_range(ireg.V, DOUBLE_MAN, DOUBLE_MAN + DOUBLE_EXP - 1);
    exp = exp_tmp - bias;
  ...
  */
  ...
  HLS_CONSTEXPR_EXTRA AP_INLINE AP_NODEBUG ap_fixed_base& equal_constexpr(
    ...
    if (!op) Base::V = 0;
    bool carry = false;
    bool signbit = _AP_ROOT_op_get_bit_for_constexpr(op.V, _AP_W2 - 1);
    ...
    if (F2 == _AP_F)
      ...
      if (_AP_Q != AP_TRN && !(_AP_Q == AP_TRN_ZERO && !_AP_S2)) {
        bool qbit = _AP_ROOT_op_get_bit_for_constexpr(op.V, F2 - _AP_F - 1);
        // bit after LSB.
        bool qb = (F2 - _AP_F > _AP_W2) ? _AP_S2 && signbit : qbit;
        enum { hi = ((F2 - _AP_F - 2) < _AP_W2) ? (F2 - _AP_F - 2) : (_AP_W2 - 1) };
        // bits after qb.
        bool r = (F2 > _AP_F + 1) ? (_AP_ROOT_op_get_range_for_constexpr(op.V, _AP_W2, 0, hi) != 0) : false;
        carry = quantization_adjust(qb, r, isneg);
      }
    } else {
      ...
      if (sh_amt < _AP_W) {
        if (_AP_W > _AP_W2) {
          ...
          Base::V <<= sh_amt;
#else
          unsigned int mask __attribute__((bitwidth(_AP_W))) = Base::V;
          Base::V = mask << sh_amt;
#endif
        } else {
          // shift and truncate.
          ...
          Base::V = op.V << sh_amt;
#else
          unsigned int mask __attribute__((bitwidth(_AP_W2))) = op.V;
          Base::V = mask << sh_amt;
#endif
        }
      } else {
    ...
    if ((_AP_O != AP_WRAP || _AP_N != 0) &&
      ...
      bool neg_src = isneg;
      bool newsignbit = _AP_ROOT_op_get_bit_for_constexpr(Base::V, _AP_W - 1);
      enum { pos1 = F2 - _AP_F + _AP_W, pos2 = F2 - _AP_F + _AP_W + 1 };
      bool lD = (pos1 < _AP_W2 && pos1 >= 0) ? _AP_ROOT_op_get_bit_for_constexpr(op.V, pos1)
                                             : false;
      if (pos1 < _AP_W2) {
        bool Range1_all_ones = true;
        bool Range1_all_zeros = true;
        ...
        if (pos2 < _AP_W2 && pos2 >= 0) {
          ...
          Range2.V = _AP_ROOT_op_get_range_for_constexpr(op.V, _AP_W2, pos2, _AP_W2 - 1);
          Range2_all_ones = Range2 == (all_ones >> pos2);
        } else if (pos2 < 0) {
          Range2_all_ones = false;
        }
        ...
        if (pos1 >= 0 && pos2 < _AP_W2) {
          ...
          Range1.V = _AP_ROOT_op_get_range_for_constexpr(op.V, _AP_W2, pos1, _AP_W2 - 1);
      ...
      if ((_AP_O == AP_SAT_SYM) && _AP_S2 && _AP_S)
        underflow |=
            neg_src &&
            (_AP_W > 1 ? _AP_ROOT_op_get_range_for_constexpr(Base::V, _AP_W, 0, _AP_W - 2) == 0
                       : true);
  ...
  HLS_CONSTEXPR_EXTRA AP_INLINE AP_NODEBUG void checkOverflowCsimFix(int _ap_w2, bool _ap_s2) const {
      ...
      int count = 0;
      int valid_width = 0;
      int target_width = ap_w_l;
      if (ap_s_l == true) {
          target_width = ap_w_l - 1;
      }
      if (ap_s_r == true) {
        bool sign = get_bit(i);
        if (sign == true && ap_s_l == false) {
            AP_WARNING("negative", to_float(), "ap_uint", ap_w_l);
            return;
        }
        if(sign == true) {
          count = countLeadingOnes();
        } else {
          count = countLeadingZeros();
        }
        valid_width = ap_w_r - count;
        if(valid_width - target_width > 0) {
          AP_WARNING("overflow", to_float(), "ap_int", ap_w_l);
        }
      } else {
        count = countLeadingZeros();
        valid_width = ap_w_r - count;
        if(valid_width - target_width > 0) {
          AP_WARNING("overflow", to_float(), "ap_uint", ap_w_l);
        }
      }
  #else
      (void)_ap_w2;
    ...
    if (Cnative && _AP_I < _AP_W) {
      ...
      if (_AP_S && _AP_ROOT_op_get_bit(Base::V, _AP_W - 1) && (_AP_I < _AP_W) &&
          (_AP_ROOT_op_get_range(
               Base::V, 0, _AP_I < 0 ? _AP_W - 1 : _AP_W - _AP_I - 1) != (unsigned long)0))
        //++ret;
        ret = ret + 1;
    } else {
...
// TODO check shift overflow?
    ...
    if (_AP_O != AP_WRAP || _AP_N != 0) {
      ...
      bool allones(0), allzeros(0);
      ap_int_base<_AP_W, false> ones(-1);
      if (sh <= _AP_W) {
        ap_int_base<_AP_W, false> range1(0);
...
AF_BIN_OP_WITH_AP_INT(/, div)    //double check
...
AF_ASSIGN_OP_WITH_AP_INT(-=)
AF_ASSIGN_OP_WITH_AP_INT(*=)
AF_ASSIGN_OP_WITH_AP_INT(/=)    //double check
AF_ASSIGN_OP_WITH_AP_INT(&=)
AF_ASSIGN_OP_WITH_AP_INT(|=)
AF_ASSIGN_OP_WITH_AP_INT(^=)
