...
// for hls_constexpr
#if (defined HLS_CONSTEXPR_INTERNAL_OPEN)
#if (!defined __SYNTHESIS__) && (defined(__GNUC__) && (__GNUC__ >= 6)) && (__cplusplus >= 201402L) 
#define HLS_CONSTEXPR_ENABLE
#endif
#if (defined __SYNTHESIS__) && (defined(__clang__) && ((__clang_major__ > 3) || ((__clang_major__ == 3) &&(__clang_minor__ > 3)))) && (__cplusplus >= 201402L) 
#define HLS_CONSTEXPR_ENABLE
#endif
#endif

#if (defined HLS_CONSTEXPR_ENABLE)
#define HLS_CONSTEXPR constexpr
#else 
#define HLS_CONSTEXPR
#endif

#if (defined HLS_CONSTEXPR_INTERNAL_OPEN) 
#if (!defined HLS_CONSTEXPR_ENABLE) && (defined HLS_CONSTEXPR_USER_ENABLE)
#warning "Please make sure the constexpr is enabled"
#elif (defined HLS_CONSTEXPR_ENABLE) && (defined HLS_CONSTEXPR_USER_ENABLE)
#define HLS_CONSTEXPR_EXTRA_ENABLE
#endif
#endif 

#if (defined HLS_CONSTEXPR_EXTRA_ENABLE)
#define HLS_CONSTEXPR_EXTRA constexpr
#else 
#define HLS_CONSTEXPR_EXTRA 
#endif 
...
#define AP_WARNING(OVERFLOW_TYPE, VALUE, AP, WIDTH)                            \
    std::cout << "WARNING: assign " << OVERFLOW_TYPE << " value "              \
        << VALUE << " to type " << AP << "<" << WIDTH << ">."                  \
        << std::endl;                                                          \
    std::cout << "Use gdb to diagnose into " << __FILE__ <<                    \
...
#define _AP_ROOT_op_get_bit_for_constexpr(Val, Bit) (((Val) >> ((unsigned int)Bit)) & 1Ull)
#define _AP_ROOT_op_get_range_for_constexpr(Val, bitw, Lo, Hi)                   \
  ({                                                                            \
    typename _ap_type::remove_const<typeof(Val)>::type __Val2__ = Val;          \
    unsigned int  mask __attribute__((bitwidth((bitw)))) = -1Ull;               \
    mask >>= (int)((bitw) - ((Hi) - (Lo) + 1));                                 \
    __Val2__ >>= (int)(Lo);                                                     \
    __Val2__ &= mask;                                                           \
    __Val2__;                                                                   \
  })
...
struct ssdm_int_sim {
  ...
  HLS_CONSTEXPR ssdm_int_sim(ap_private<_AP_W, _AP_S> o):V(o){}
};
...
#define _AP_ROOT_op_get_bit_for_constexpr _AP_ROOT_op_get_bit
#define _AP_ROOT_op_get_range_for_constexpr(Val, bitw, Lo, Hi) _AP_ROOT_op_get_range(Val, Lo, Hi)
#define _AP_ROOT_op_get_bit(Val, Bit) (Val).get_bit((Bit))
template <typename _Tp1, typename _Tp2, typename _Tp3>
HLS_CONSTEXPR_EXTRA inline _Tp1& _AP_ROOT_op_set_bit(_Tp1& Val, const _Tp2& Bit, const _Tp3& Repl) {
...
HLS_CONSTEXPR_EXTRA inline _Tp1& _AP_ROOT_op_set_range(_Tp1& Val, const _Tp2& Lo, const _Tp3& Hi,
                                   const _Tp4& Repl) {
