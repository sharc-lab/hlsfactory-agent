...
public:
  ...
  // Provide easy way to do type conversion, especially for 3 floating-point
  // type specializations, i.e., std::complex<float|double|long double>
  template<class T>
  AP_NODEBUG AP_INLINE complex<T> to() {
    complex<_Tp> tmp(*this);
    return complex<T>(tmp.real(), tmp.imag());
  }
