...
#include "etc/hls_stream_common.h"
...
template<size_t _SIZE, bool _DIRECTIO>
class stream_entity {
public:
  ...
  bool read(void *elem) {
    ...
    stream_globals<_DIRECTIO>::start_threads();
    ...
    if (data.empty()) {
      ...
      if (allow_read_empty) {
        ...
        stream_globals<_DIRECTIO>::incr_blocked_counter();
        while (data.empty()) {
#ifndef HLS_STREAM_THREAD_UNSAFE
          while (invalid) { 
        ...
        stream_globals<_DIRECTIO>::decr_blocked_counter();
      }
    }
    std::array<char, _SIZE> &elem_data = data.front();
    memcpy(elem, elem_data.data(), _SIZE);
    data.pop_front();
    return true;
  }
  ...
  void write(const void *elem) {
    ...
    std::array<char, _SIZE> elem_data;
    memcpy(elem_data.data(), elem, _SIZE);
    ...
    if (stream_globals<_DIRECTIO>::get_max_size() < data.size()) 
        stream_globals<_DIRECTIO>::get_max_size() = data.size();
#ifndef HLS_STREAM_THREAD_UNSAFE
  ...
  stream_delegate<_SIZE> *d;
  std::string name;
  std::deque<std::array<char, _SIZE> > data;
#ifndef HLS_STREAM_THREAD_UNSAFE
  std::mutex mutex;
...
public:
  ...
  static stream_entity<_SIZE, _DIRECTIO> &get_entity(void *p) {
#ifndef HLS_STREAM_THREAD_UNSAFE
    std::lock_guard<std::mutex> lg(get_mutex());
#endif
    return get_map()[p];
...
#endif
  static std::unordered_map<void*, stream_entity<_SIZE, _DIRECTIO> > &get_map() {
    static std::unordered_map<void*, stream_entity<_SIZE, _DIRECTIO> > *map = 
        new std::unordered_map<void*, stream_entity<_SIZE, _DIRECTIO> >();
    return *map;
  }
};
