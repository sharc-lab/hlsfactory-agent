...
#if !defined(__HLS_CSIM__) && defined(__HLS_COSIM__) 
extern "C"
...
class autorestart {
public:
  static unsigned getOrSetExecuteStatus(unsigned newVal, bool set) {
     static unsigned executed = 0; 
     if (set)
        executed = newVal;
     return executed;
  }
  ...
  template <class T, class... Args>
  autorestart(int count, T fn, Args&&... args) {
#if !defined(__HLS_CSIM__) && defined(__HLS_COSIM__) 
    __hls_sim_static_autorestart_seq_push(count);
#endif
#ifndef __SYNTHESIS__
    if (count <= 0) {
        std::cerr << "ERROR: the count argument for hls::autorestart must be a positive integer " << std::endl;
        exit(1);
    }
    for (int i = 0; i < count; i++) {
      fn(args...);
    }
#if defined(__clang_major__) && __clang_major__ == 16 && (defined(__HLS_CSIM__))
    if (!getOrSetExecuteStatus(0, false) && count > 0) {
        std::cerr << "ERROR: the kernel function called by hls::autorestart does not have '#pragma HLS interface s_axilite port=return autorestart' " << std::endl;
        exit(1);
    }
#endif
#endif
  ...
  template <class T, class... Args>
  void operator()(int count, T fn, Args&&... args) {
#if !defined(__HLS_CSIM__) && defined(__HLS_COSIM__) 
    __hls_sim_static_autorestart_seq_push(count);
#endif
#ifndef __SYNTHESIS__
