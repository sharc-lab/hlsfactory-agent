...
ap_uint<48> WLM_mul_default_multiplier_24x24(ap_uint<24> u, ap_uint<24> v){
  #pragma HLS inline

  ap_uint<17> u_0 = u & ((1<<17)-1);
  u = u >> 17;
  ap_uint<7> u_1 = u & ((1<<7)-1);

  ap_uint<24> v_0 = v & ((1<<24)-1);
  
  ap_uint<41> mul_0 = ((ap_uint<41>)(u_0) * v_0); //<<0
  #pragma HLS bind_op variable=mul_0 op=mul impl=dsp
  ap_uint<31> mul_1 = ((ap_uint<31>)(u_1) * v_0); //<<17
  #pragma HLS bind_op variable=mul_1 op=mul impl=dsp

  
  ap_uint<48> res = ( (((ap_uint<48>)mul_0)<<0) + (((ap_uint<48>)mul_1)<<17) );
  return res;
}


WORD wlm_reduce(WORD u, WORD v, WORD q, WORD multiplier) 
{
  #pragma HLS inline
  ...
  //==== Iteration 0 ====
  DWORD t0_h = product_uv >> 24;
  WLM_WORD t0_l = product_uv % (((WORD)1)<<24);

  WLM_WORD t0_i = ((((WORD)1)<<24) - t0_l) % (((WORD)1)<<24);
  
  WLM_WORD carry_0 = t0_i.range(23,23) | t0_l.range(23,23);

  DWORD t1 = t0_h + WLM_mul_default_multiplier_24x24(multiplier, t0_i) + carry_0;

  //==== Iteration 1 ====
  DWORD t1_h = t1 >> 24;
  WLM_WORD t1_l = t1 % (((WORD)1)<<24);

  WLM_WORD t1_i = ((((WORD)1)<<24) - t1_l) % (((WORD)1)<<24);
  
  WLM_WORD carry_1 = t1_i.range(23,23) | t1_l.range(23,23);
  
  DWORD t2 = t1_h + WLM_mul_default_multiplier_24x24(multiplier, t1_i) + carry_1;
...
void BU(
    ...
    WORD q,
    WORD twoInverse,
    WLM_MULTIPLIER_WORD multiplier,
    bool direction,
    VAR_TYPE_16 iter,
    int BUIdx
    ){
  ...
  BU_COMP:for (;i<numBUIter; ) {
    ...
    if((((!fwd_inVal0.empty()) && (!fwd_inVal1.empty())) || ((!inv_inVals[0].empty()) && (!inv_inVals[1].empty())))  && (!inTf.empty())){
      ...
      WORD reducedProduct = wlm_reduce(right, tfVal, q, multiplier);
    ...
    )
{
    ...
    tapa::task()
        ...
        //BU tasks
        .invoke(BU, fwd_polyBuf_to_BU_L0_0[0], fwd_polyBuf_to_BU_L0_2[0], inv_polyBuf_to_BU_L0_0, tfBuf_to_BU_L0_0[0], fwd_BU_to_polyBuf_L0_0, inv_BU_to_polyBuf_L0_0[0], inv_BU_to_polyBuf_L0_2[0], q0, twoInverse0, multiplier0, direction, iter, 0)
        .invoke(BU, fwd_polyBuf_to_BU_L0_0[1], fwd_polyBuf_to_BU_L0_2[1], inv_polyBuf_to_BU_L0_1, tfBuf_to_BU_L0_1[0], fwd_BU_to_polyBuf_L0_1, inv_BU_to_polyBuf_L0_0[1], inv_BU_to_polyBuf_L0_2[1], q0, twoInverse0, multiplier0, direction, iter, 1)
        .invoke(BU, fwd_polyBuf_to_BU_L0_1[0], fwd_polyBuf_to_BU_L0_3[0], inv_polyBuf_to_BU_L0_2, tfBuf_to_BU_L0_0[1], fwd_BU_to_polyBuf_L0_2, inv_BU_to_polyBuf_L0_1[0], inv_BU_to_polyBuf_L0_3[0], q0, twoInverse0, multiplier0, direction, iter, 2)
        .invoke(BU, fwd_polyBuf_to_BU_L0_1[1], fwd_polyBuf_to_BU_L0_3[1], inv_polyBuf_to_BU_L0_3, tfBuf_to_BU_L0_1[1], fwd_BU_to_polyBuf_L0_3, inv_BU_to_polyBuf_L0_1[1], inv_BU_to_polyBuf_L0_3[1], q0, twoInverse0, multiplier0, direction, iter, 3)
