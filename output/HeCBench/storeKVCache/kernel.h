    ...
    const uint64_t num_items)
{
  ...
  for (uint64_t i = 0; i < num_items; ++i) {
    reinterpret_cast<ulong2*>(k_dst)[lane_id + i * WARP_SIZE] =
      reinterpret_cast<const ulong2*>(k_src)[lane_id + i * WARP_SIZE];
    reinterpret_cast<ulong2*>(v_dst)[lane_id + i * WARP_SIZE] =
      reinterpret_cast<const ulong2*>(v_src)[lane_id + i * WARP_SIZE];
  }
}
    ...
    const uint64_t num_items)
{
  ...
  for (uint64_t i = 0; i < num_items; ++i) {
    reinterpret_cast<ulong4*>(k_dst)[lane_id + i * WARP_SIZE] =
      reinterpret_cast<const ulong4*>(k_src)[lane_id + i * WARP_SIZE];
    reinterpret_cast<ulong4*>(v_dst)[lane_id + i * WARP_SIZE] =
      reinterpret_cast<const ulong4*>(v_src)[lane_id + i * WARP_SIZE];
  }
}
