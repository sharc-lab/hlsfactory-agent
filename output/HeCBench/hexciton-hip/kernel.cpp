...
void comm_empty(
    real_2_t *  sigma_in,
    real_2_t *  sigma_out, 
    real_2_t *  hamiltonian)
{
    #pragma HLS INTERFACE m_axi port=sigma_in offset=slave bundle=gmem0
    #pragma HLS INTERFACE m_axi port=sigma_out offset=slave bundle=gmem1
    #pragma HLS INTERFACE m_axi port=hamiltonian offset=slave bundle=gmem2
...
void comm_init (
    const real_2_t *  sigma_in,
          real_2_t *  sigma_out, 
    const real_2_t *  hamiltonian,
    const int dim)
{
    #pragma HLS INTERFACE m_axi port=sigma_in offset=slave bundle=gmem0
    #pragma HLS INTERFACE m_axi port=sigma_out offset=slave bundle=gmem1
...
void comm_refactor(
    const real_2_t *  sigma2_in,
          real_2_t *  sigma2_out, 
    const real_2_t *  hamiltonian2,
    const int dim)
{
    #pragma HLS INTERFACE m_axi port=sigma2_in offset=slave bundle=gmem0
    #pragma HLS INTERFACE m_axi port=sigma2_out offset=slave bundle=gmem1
    #pragma HLS INTERFACE m_axi port=hamiltonian2 offset=slave bundle=gmem2
    #pragma HLS INTERFACE s_axilite port=dim
    #pragma HLS INTERFACE s_axilite port=return
        ...
        for (int _tid_y = 0; _tid_y < BLOCK_DIM_Y; _tid_y++) {
            for (int _bid_x = 0; _bid_x < GRID_DIM_X; _bid_x++) {
                ...
                #pragma HLS PIPELINE II=1
                    ...
                    real_t * sigma_in = (real_t*) sigma2_in;
                    real_t * sigma_out = (real_t*) sigma2_out;
                    real_t * hamiltonian = (real_t*) hamiltonian2;
        ...
        for (int _tid_y = 0; _tid_y < BLOCK_DIM_Y; _tid_y++) {
            for (int _bid_x = 0; _bid_x < GRID_DIM_X; _bid_x++) {
                ...
                #pragma HLS PIPELINE II=1
                    ...
                    #define package_id ((PACKAGES_PER_WG * _bid_y + _tid_y) * (VEC_LENGTH_AUTO * 2 * dim * dim))
                    #define sigma_id _tid_x
        ...
        for (int _tid_y = 0; _tid_y < BLOCK_DIM_Y; _tid_y++) {
            for (int _bid_x = 0; _bid_x < GRID_DIM_X; _bid_x++) {
                ...
                #pragma HLS PIPELINE II=1
                    ...
                    real_vec_t * sigma_in = (real_vec_t*) sigma2_in;
                    real_vec_t * sigma_out = (real_vec_t*) sigma2_out;
                    real_t * hamiltonian = (real_t*) hamiltonian2;
    ...
    const int num)
{
    ...
    #pragma HLS INTERFACE s_axilite port=return
    #pragma HLS ARRAY_PARTITION variable=ham_local_real complete dim=1
    #pragma HLS ARRAY_PARTITION variable=ham_local_imag complete dim=1
    #pragma HLS ARRAY_PARTITION variable=sigma_local_real complete dim=1
    #pragma HLS ARRAY_PARTITION variable=sigma_local_imag complete dim=1
        ...
        for (int _tid_y = 0; _tid_y < BLOCK_DIM_Y; _tid_y++) {
            for (int _bid_x = 0; _bid_x < GRID_DIM_X; _bid_x++) {
                ...
                #pragma HLS PIPELINE II=1
                    ...
                    int sub_group_id = _tid_y; // Local matrix ID within work group
                    int start = _bid_x * NUM_SUB_GROUPS * CHUNK_SIZE + sub_group_id * CHUNK_SIZE; // Global matrix ID : start
