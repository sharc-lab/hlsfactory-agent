...
void Hamiltonian::initialize_gpu(Model& model)
{
  ...
  CHECK(
    hipMemcpy(neighbor_number, model.neighbor_number, sizeof(int) * n, hipMemcpyHostToDevice));
  delete[] model.neighbor_number;
  CHECK(hipMemcpy(potential, model.potential, sizeof(real) * n, hipMemcpyHostToDevice));
  delete[] model.potential;

  int* neighbor_list_new = new int[model.number_of_pairs];
  delete[] model.neighbor_list;
  CHECK(hipMemcpy(
    neighbor_list, neighbor_list_new, sizeof(int) * model.number_of_pairs, hipMemcpyHostToDevice));
  delete[] neighbor_list_new;

  real* hopping_real_new = new real[model.number_of_pairs];
  delete[] model.hopping_real;
  CHECK(hipMemcpy(
    hopping_real, hopping_real_new, sizeof(real) * model.number_of_pairs, hipMemcpyHostToDevice));
  delete[] hopping_real_new;

  real* hopping_imag_new = new real[model.number_of_pairs];
  delete[] model.hopping_imag;
  CHECK(hipMemcpy(
    hopping_imag, hopping_imag_new, sizeof(real) * model.number_of_pairs, hipMemcpyHostToDevice));
  delete[] hopping_imag_new;

  real* xx_new = new real[model.number_of_pairs];
  delete[] model.xx;
  CHECK(hipMemcpy(xx, xx_new, sizeof(real) * model.number_of_pairs, hipMemcpyHostToDevice));
  delete[] xx_new;
}
#else
...
{
#ifndef CPU_ONLY
  hipLaunchKernelGGL(gpu_apply_hamiltonian, dim3(grid_size), dim3(BLOCK_SIZE), 0, 0, 
...
{
#ifndef CPU_ONLY
  hipLaunchKernelGGL(gpu_apply_commutator, dim3(grid_size), dim3(BLOCK_SIZE), 0, 0, 
...
{
#ifndef CPU_ONLY
  hipLaunchKernelGGL(gpu_apply_current, dim3(grid_size), dim3(BLOCK_SIZE), 0, 0, 
...
{
#ifndef CPU_ONLY
  hipLaunchKernelGGL(gpu_chebyshev_01, dim3(grid_size), dim3(BLOCK_SIZE), 0, 0, 
...
{
#ifndef CPU_ONLY
  hipLaunchKernelGGL(gpu_chebyshev_2, dim3(grid_size), dim3(BLOCK_SIZE), 0, 0, 
    n, energy_max, neighbor_number, neighbor_list, potential, hopping_real, hopping_imag,
...
{
#ifndef CPU_ONLY
  hipLaunchKernelGGL(gpu_chebyshev_1x, dim3(grid_size), dim3(BLOCK_SIZE), 0, 0, 
...
{
#ifndef CPU_ONLY
  hipLaunchKernelGGL(gpu_chebyshev_2x, dim3(grid_size), dim3(BLOCK_SIZE), 0, 0, 
...
{
#ifndef CPU_ONLY
  hipLaunchKernelGGL(gpu_kernel_polynomial, dim3(grid_size), dim3(BLOCK_SIZE), 0, 0, 
    n, energy_max, neighbor_number, neighbor_list, potential, hopping_real, hopping_imag,
    state_0.real_part, state_0.imag_part, state_1.real_part, state_1.imag_part, state_2.real_part,
    state_2.imag_part);
  CHECK(hipGetLastError());
