...
#pragma unroll
  ...
  static float warpLevelSums[SHARED_MEM_MAX_ROWS][WarpSize];
  const int laneId = _tid_x % WarpSize;
  const int warpId = _tid_x / WarpSize;
  if (laneId == 0) warpLevelSums[_tid_y][warpId] = sum;
  // read from shared memory only if that warp existed
  sum = (_tid_x < BLOCK_DIM_X / WarpSize)
            ? warpLevelSums[_tid_y][laneId]
            : 0.0;
  // Final reduce using first warp
  if (warpId == 0) sum = warpReduceSum(sum, BLOCK_DIM_X / WarpSize);
  if (tid == 0) {
    res[row] = __float2half(sum);
  }
}
