...
T parallel_prefix_sum(const int n, const int *ind, const T *w)
{
  ...
  for (int i=_tid_x; i<mn; i+=BLOCK_DIM_X) {
    ...
    last = __shfl(sum, BLOCK_DIM_X-1, BLOCK_DIM_X);
    ...
    for (int j=1; j<BLOCK_DIM_X; j*=2) {
      T v = __shfl_up(sum, j, BLOCK_DIM_X);
      if (_tid_x >= j) sum += v;
    }
    //shift by last
    sum += last;
                ...
                for (int _tid_y = 0; _tid_y < BLOCK_DIM_Y; _tid_y++) {
                    for (int _bid_x = 0; _bid_x < GRID_DIM_X; _bid_x++) {
                        ...
                        #pragma HLS PIPELINE II=1
                            ...
                            for (int j=csrPtr[row]+_tid_y+_bid_y*BLOCK_DIM_Y; j<csrPtr[row+1]; j+=GRID_DIM_Y*BLOCK_DIM_Y) {
                            int col = csrInd[j];
