...
// --- from kernel.cu ---
short blockShuffleReduce_with_index(short myVal, short& myIndex, short& myIndex2, unsigned lengthSeqB, bool inverse)
{
  int              laneId = _tid_x % 32;
  int              warpId = _tid_x / 32;
  short locTots[32];
  short locInds[32];
  short locInds2[32];
  short            myInd  = myIndex;
  ...
  unsigned nblocks = ((32 + BLOCK_DIM_X - 1) / 32);
  if(_tid_x < nblocks)
  {
    myVal  = locTots[_tid_x];
    myInd  = locInds[_tid_x];
    myInd2 = locInds2[_tid_x];
  }
  else
  {
    myVal  = 0;
    ...
    const short* encoding_matrix)
{
    ...
    #pragma HLS INTERFACE m_axi port=seqA_array offset=slave bundle=gmem0
    #pragma HLS INTERFACE m_axi port=seqB_array offset=slave bundle=gmem1
    #pragma HLS INTERFACE m_axi port=prefix_lengthA offset=slave bundle=gmem2
    #pragma HLS INTERFACE m_axi port=prefix_lengthB offset=slave bundle=gmem3
    #pragma HLS INTERFACE m_axi port=seqA_align_begin offset=slave bundle=gmem4
    #pragma HLS INTERFACE m_axi port=seqA_align_end offset=slave bundle=gmem5
    #pragma HLS INTERFACE m_axi port=seqB_align_begin offset=slave bundle=gmem6
    #pragma HLS INTERFACE m_axi port=seqB_align_end offset=slave bundle=gmem7
    #pragma HLS INTERFACE m_axi port=top_scores offset=slave bundle=gmem8
    #pragma HLS INTERFACE s_axilite port=startGap
    #pragma HLS INTERFACE s_axilite port=extendGap
    #pragma HLS INTERFACE m_axi port=scoring_matrix offset=slave bundle=gmem9
    #pragma HLS INTERFACE m_axi port=encoding_matrix offset=slave bundle=gmem10
    #pragma HLS INTERFACE s_axilite port=return
    #pragma HLS ARRAY_PARTITION variable=is_valid_array complete dim=1
    #pragma HLS ARRAY_PARTITION variable=sh_prev_E complete dim=1
    #pragma HLS ARRAY_PARTITION variable=sh_prev_H complete dim=1
    #pragma HLS ARRAY_PARTITION variable=sh_prev_prev_H complete dim=1
    #pragma HLS ARRAY_PARTITION variable=local_spill_prev_E complete dim=1
    #pragma HLS ARRAY_PARTITION variable=local_spill_prev_H complete dim=1
    #pragma HLS ARRAY_PARTITION variable=local_spill_prev_prev_H complete dim=1
    #pragma HLS ARRAY_PARTITION variable=sh_aa_encoding complete dim=1
    #pragma HLS ARRAY_PARTITION variable=sh_aa_scoring complete dim=1
    ...
    for (int _bid_x = 0; _bid_x < GRID_DIM_X; _bid_x++) {
        ...
        #pragma HLS PIPELINE II=1
            ...
            short laneId = _tid_x%32;
            short warpId = _tid_x/32;
            ...
            short sh_prev_E[32]; // one such element is required per warp
            short sh_prev_H[32];
            short sh_prev_prev_H[32];

            short local_spill_prev_E[1024];// each threads local spill,
            short local_spill_prev_H[1024];
            short local_spill_prev_prev_H[1024];

            short sh_aa_encoding[ENCOD_MAT_SIZE];// length = 91
            short sh_aa_scoring[SCORE_MAT_SIZE];

            int max_threads = BLOCK_DIM_X;
            ...
            {
            unsigned mask  = 0, (is_valid[thread_Id] &&( thread_Id < minSize)));
