const int cNSIZE = 2003;           // Values
const int cMSIZE = cNSIZE*3+3;      // Gradient vectors
const int cOSIZE = cNSIZE*9+9;      // Hessian Matrices 

static inline void eval_ref (
    ...
    const int iz) 
{
  for (int n = 0; n < spline_num_splines; n++) {
    eval_ref ( 
        ...
        xs, ys, zs, 
        walkers_vals+i*cNSIZE+n,
        walkers_grads+i*cMSIZE+n*3,
        walkers_hess+i*cOSIZE+n*9,
        a,
        b,
        c,
        da,
