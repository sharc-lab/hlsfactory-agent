...
const float gradientX[N_GRADIENTS] = {
  1, -1, 0, 0, INVSQRT2, -INVSQRT2, INVSQRT2, -INVSQRT2
};

const float gradientY[N_GRADIENTS] = {
...
void Perlin::calculate(uint8_t *hPixels, NoiseParams params, hipStream_t *streams, int nStreams) {
  ...
  uint8_t *dPixels;
  CHECK(hipMalloc((void**)&dPixels, sizeof(uint8_t) * 4 * WIN_WIDTH * WIN_HEIGHT));
  ...
  CHECK(hipMemcpy(hPixels, dPixels, sizeof(uint8_t) * 4 * WIN_WIDTH * WIN_HEIGHT,
        hipMemcpyDeviceToHost));

  CHECK(hipFree(dPixels));
}
