...
#define HIP_THREADS_NO_MAC 1024
...
int evaluate_w_block(unsigned char * salt, uint32_t * d_w_words_uint32);
...
#define ROR19(x) (((x) << 13) | ((x) >> 19))
		...
		static inline uint32_t LOP3LUT_XOR(uint32_t a, uint32_t b, uint32_t c) {
			uint32_t d;
			asm("lop3.b32 %0, %1, %2, %3, 0x96;" : "=r"(d) : "r"(a), "r"(b), "r"(c));
			return d;
		}

		static inline uint32_t LOP3LUT_XORAND(uint32_t a, uint32_t b, uint32_t c) {
			uint32_t d;
			asm("lop3.b32 %0, %1, %2, %3, 0xb8;" : "=r"(d) : "r"(a), "r"(b), "r"(c));
			return d;
		}

		static inline uint32_t LOP3LUT_ANDOR(uint32_t a, uint32_t b, uint32_t c) {
			uint32_t d;
        ...
        static inline uint32_t LOP3LUT_XORAND(uint32_t g, uint32_t e, uint32_t f) {
            return (g ^ (e & (f ^ g)));
        }
