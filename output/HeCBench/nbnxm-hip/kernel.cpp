...
static inline void reduceForceIAndFShiftXYZ(
    float* sm_buf,
    const float* fCiBufX,
    const float* fCiBufY,
    const float* fCiBufZ,
  ...
  if (calcFShift)
  {
    ...
    if (tidxj < 3)
    {
      if constexpr (c_clSize == 4)
      {
        ...
        if (tidxi == 0)
        {
          (a_fShift[shift][tidxj] += fShiftBuf);
        }
      }
      else
      {
...
static inline void reduceForceJShuffle(
    Float3 f,
    const int tidxi,
    const int aidx,
    Float3 *a_f)
...
static inline void reduceForceJ(
    float* sm_buf,
    Float3 f,
    const int tidxi,
    const int tidxj,
...
{
    #pragma HLS INTERFACE m_axi port=a_xq offset=slave bundle=gmem0
    #pragma HLS INTERFACE m_axi port=a_f offset=slave bundle=gmem1
    #pragma HLS INTERFACE m_axi port=a_shiftVec offset=slave bundle=gmem2
    #pragma HLS INTERFACE m_axi port=a_fShift offset=slave bundle=gmem3
    #pragma HLS INTERFACE m_axi port=a_plistCJ4 offset=slave bundle=gmem4
    #pragma HLS INTERFACE m_axi port=a_plistSci offset=slave bundle=gmem5
    #pragma HLS INTERFACE m_axi port=a_plistExcl offset=slave bundle=gmem6
    #pragma HLS INTERFACE m_axi port=a_atomTypes offset=slave bundle=gmem7
    #pragma HLS INTERFACE m_axi port=a_nbfp offset=slave bundle=gmem8
    #pragma HLS INTERFACE s_axilite port=numTypes
    #pragma HLS INTERFACE s_axilite port=rCoulombSq
    #pragma HLS INTERFACE s_axilite port=ewaldBeta
    #pragma HLS INTERFACE s_axilite port=epsFac
    #pragma HLS INTERFACE s_axilite port=calcShift
    #pragma HLS INTERFACE s_axilite port=return
    #pragma HLS ARRAY_PARTITION variable=localPtr complete dim=1
    ...
    for (int _bid_z = 0; _bid_z < GRID_DIM_Z; _bid_z++) {
        for (int _tid_y = 0; _tid_y < BLOCK_DIM_Y; _tid_y++) {
            ...
            #pragma HLS PIPELINE II=1
                ...
                const unsigned tidxi = _tid_x;
                const unsigned tidxj = _tid_y;
                const unsigned tidx  = tidxj * c_clSize + tidxi;
                const unsigned bidx = _bid_z;
                ...
                uint8_t localPtr[local_mem_size];
