                               ...
                               const float*  bias, int N, int C)
{
    ...
    for (int _bid_x = 0; _bid_x < GRID_DIM_X; _bid_x++) {
        ...
        #pragma HLS PIPELINE II=1
            ...
            const float mean_sum = BlockReduce<SumOp, float, THREADS_PER_WARP>(local_sum) / static_cast<float>(C);
            if (_tid_x == 0)
            ...
            const float mean_square = BlockReduce<SumOp, float, THREADS_PER_WARP>(local_sum) / static_cast<float>(C);
