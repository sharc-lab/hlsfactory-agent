...
void genScoreKernel(const int sizepernode, 
                               ...
                               const float *D_LG)
{
    #pragma HLS INTERFACE s_axilite port=sizepernode
    #pragma HLS INTERFACE m_axi port=D_localscore offset=slave bundle=gmem0
    #pragma HLS INTERFACE m_axi port=D_data offset=slave bundle=gmem1
    #pragma HLS INTERFACE m_axi port=D_LG offset=slave bundle=gmem2
...
void computeKernel(const int taskperthr,
                              ...
                              int *D_resP)
{
    #pragma HLS INTERFACE s_axilite port=taskperthr
    #pragma HLS INTERFACE s_axilite port=sizepernode
    #pragma HLS INTERFACE m_axi port=D_localscore offset=slave bundle=gmem0
    #pragma HLS INTERFACE m_axi port=D_parent offset=slave bundle=gmem1
    #pragma HLS INTERFACE s_axilite port=node
    #pragma HLS INTERFACE s_axilite port=total
    #pragma HLS INTERFACE m_axi port=D_Score offset=slave bundle=gmem2
    #pragma HLS INTERFACE m_axi port=D_resP offset=slave bundle=gmem3
    #pragma HLS INTERFACE s_axilite port=return
    #pragma HLS ARRAY_PARTITION variable=lsinblock complete dim=1

    for (int _bid_x = 0; _bid_x < GRID_DIM_X; _bid_x++) {
        for (int _tid_x = 0; _tid_x < BLOCK_DIM_X; _tid_x++) {
        #pragma HLS PIPELINE II=1

            float lsinblock[4096];
            const unsigned int id = _bid_x*256 + _tid_x;
            const unsigned int tid = _tid_x;
            const unsigned int bid = _bid_x;
