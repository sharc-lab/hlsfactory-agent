  ...
  int iter)
{
  ...
  for (int z = _bid_z; z < nz; z += GRID_DIM_Z) {
    for (int y = _tid_y + _bid_y*BLOCK_DIM_Y; y < ny; y += BLOCK_DIM_Y*GRID_DIM_Y) {
      for (int x = _tid_x + _bid_x*BLOCK_DIM_X; x < nx; x += BLOCK_DIM_X*GRID_DIM_X) {
