...
{
  int _idx = _bid_x * BLOCK_DIM_X + _tid_x;
  if (_idx >= _N) return;
  float h = d_h[_idx];
  float m = d_m[_idx];
  float n = d_n[_idx];
...
void neurongroup_stateupdater (
    float*   _ptr_array_neurongroup_ge,
    float*   _ptr_array_neurongroup_gi,
    float*   _ptr_array_neurongroup_h,
    float*   _ptr_array_neurongroup_m,
    float*   _ptr_array_neurongroup_n,
    float*   _ptr_array_neurongroup_v,
    float*    _ptr_array_neurongroup_lastspike,
    float*   _ptr_array_defaultclock_dt,
    float*  _ptr_array_defaultclock_t,
    ...
    const int iteration ) 
{
  ...
  float* d_h;
  hipMalloc((void**)&d_h, _N*sizeof(float)); 
  hipMemcpyAsync(d_h, _ptr_array_neurongroup_h, _N*sizeof(float), hipMemcpyHostToDevice, 0);

  float* d_m;
  hipMalloc((void**)&d_m, _N*sizeof(float)); 
  hipMemcpyAsync(d_m, _ptr_array_neurongroup_m, _N*sizeof(float), hipMemcpyHostToDevice, 0);

  float* d_n;
  hipMalloc((void**)&d_n, _N*sizeof(float)); 
  hipMemcpyAsync(d_n, _ptr_array_neurongroup_n, _N*sizeof(float), hipMemcpyHostToDevice, 0);

  float* d_ge;
  hipMalloc((void**)&d_ge, _N*sizeof(float)); 
  hipMemcpyAsync(d_ge, _ptr_array_neurongroup_ge, _N*sizeof(float), hipMemcpyHostToDevice, 0);

  float* d_v;
  hipMalloc((void**)&d_v, _N*sizeof(float)); 
  hipMemcpyAsync(d_v, _ptr_array_neurongroup_v, _N*sizeof(float), hipMemcpyHostToDevice, 0);

  float* d_gi;
  hipMalloc((void**)&d_gi, _N*sizeof(float)); 
  hipMemcpyAsync(d_gi, _ptr_array_neurongroup_gi, _N*sizeof(float), hipMemcpyHostToDevice, 0);

  float* d_lastspike;
  hipMalloc((void**)&d_lastspike, _N*sizeof(float)); 
  hipMemcpyAsync(d_lastspike, _ptr_array_neurongroup_lastspike, _N*sizeof(float), hipMemcpyHostToDevice, 0);

  char* d_not_refractory;
  hipMalloc((void**)&d_not_refractory, _N*sizeof(char)); 
  ...
  hipMemcpyAsync(_ptr_array_neurongroup_m, d_m, _N*sizeof(float), hipMemcpyDeviceToHost, 0);
  hipMemcpyAsync(_ptr_array_neurongroup_n, d_n, _N*sizeof(float), hipMemcpyDeviceToHost, 0);
  hipMemcpyAsync(_ptr_array_neurongroup_v, d_v, _N*sizeof(float), hipMemcpyDeviceToHost, 0);
  hipMemcpyAsync(_ptr_array_neurongroup_h, d_h, _N*sizeof(float), hipMemcpyDeviceToHost, 0);
  hipMemcpyAsync(_ptr_array_neurongroup_not_refractory, d_not_refractory, _N*sizeof(char), hipMemcpyDeviceToHost, 0);
  hipDeviceSynchronize();
