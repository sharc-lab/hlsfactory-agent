...
/**
 * Templated Implicit FMA HIP Kernels with Shared Memory Optimization
 *
 ...
 * - No direct HIP imports in Python bindings
 ...
 */
template <typename T>
void implicit_fma_kernel(const T*  a,
                                    const T*  b,
                                    T*  c,
 ...
 */
template <>
void implicit_fma_kernel<float>(const float*  a,
                                           const float*  b,
                                           float*  c,
                                           const int*  in_indices,
                                           const int*  out_indices,
 ...
 */
template <>
void implicit_fma_kernel<__half>(const __half*  a,
                                            const __half*  b,
                                            __half*  c,
                                            const int*  in_indices,
                                            const int*  out_indices,
 ...
 */
template <>
void implicit_fma_kernel<__hip_bfloat16>(const __hip_bfloat16*  a,
  ...
  // Shared memory for vector B
  char shared_mem_raw[4096];
  __hip_bfloat16* shared_b = reinterpret_cast<__hip_bfloat16*>(shared_mem_raw);
  ...
  // Load 8 elements at a time with alignment checking
  for (int i = tid; i < C_vec8; i += threads_per_block) {
    const __hip_bfloat16* b_ptr = &b[i * 8];
    __hip_bfloat16* sb_ptr = &shared_b[i * 8];
  ...
  if (idx < total_vec8_threads) {
    ...
    const __hip_bfloat16* a_ptr = &a[in_idx * C + vec_idx * 8];
    __hip_bfloat16* c_ptr = &c[out_idx * C + vec_idx * 8];
    const __hip_bfloat16* b_ptr = &shared_b[vec_idx * 8];
    ...
    if (reinterpret_cast<uintptr_t>(a_ptr) % 16 == 0 &&
      ...
      __hip_bfloat16* a_bfloat16s = reinterpret_cast<__hip_bfloat16*>(&a_vec);
      __hip_bfloat16* b_bfloat16s = reinterpret_cast<__hip_bfloat16*>(&b_vec);
      __hip_bfloat16* c_bfloat16s = reinterpret_cast<__hip_bfloat16*>(&c_vec);
      ...
      *reinterpret_cast<uint4*>(c_ptr) = c_vec;
      ...
      for (int i = 0; i < 8; ++i) {
        int ch_idx = vec_idx * 8 + i;
        __hip_bfloat16 a_val = a[in_idx * C + ch_idx];
        __hip_bfloat16 b_val = shared_b[ch_idx];
        c[out_idx * C + ch_idx] += a_val * b_val;
      }
    }
  } else if (C_remainder > 0) {
 ...
 */
template <typename T>
void implicit_fma_kernel_rowwise(const T*  a,
                                            const T*  b,
                                            T*  c,
                                            const int*  in_indices,
                                            const int*  out_indices,
  ...
  int op_idx = _bid_x * BLOCK_DIM_X + _tid_x;
 ...
 */
template <>
void implicit_fma_kernel_rowwise<float>(const float*  a,
                                                   const float*  b,
                                                   float*  c,
                                                   const int*  in_indices,
                                                   const int*  out_indices,
 ...
 */
template <>
void implicit_fma_kernel_rowwise<__half>(const __half*  a,
                                                    const __half*  b,
                                                    __half*  c,
                                                    const int*  in_indices,
                                                    const int*  out_indices,
 ...
 */
template <>
void implicit_fma_kernel_rowwise<__hip_bfloat16>(const __hip_bfloat16*  a,
                                                           const __hip_bfloat16*  b,
                                                           __hip_bfloat16*  c,
                                                           const int*  in_indices,
                                                           const int*  out_indices,
...
int run_implicit_fma_templated(const void* tensor_a,
  ...
  if (kernel_type == "rowwise") {
    ...
    if constexpr (std::is_same_v<ElementA, float>) {
          ...
          N_A,
          N_C);
    } else if constexpr (std::is_same_v<ElementA, __hip_bfloat16>) {
      implicit_fma_kernel_rowwise<__hip_bfloat16>

              reinterpret_cast<const __hip_bfloat16*>(a_ptr),
              reinterpret_cast<const __hip_bfloat16*>(b_ptr),
              reinterpret_cast<__hip_bfloat16*>(c_ptr),
              in_indices,
              out_indices,
              num_ops,
              C,
...
template int 
    run_implicit_fma_templated<__hip_bfloat16, __hip_bfloat16, __hip_bfloat16>(
        const void*,
        const void*,
        void*,
        const int*,
