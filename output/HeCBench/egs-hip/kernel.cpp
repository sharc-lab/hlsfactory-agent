...
indices get_indices() {
  ...
  idx.b = _bid_y * GRID_DIM_X + _bid_x;

  // index of the particle on the stack
  idx.p = idx.b * BLOCK_DIM_X + _tid_x;

  // index of the warp in the block
  idx.w = _tid_x / WARP_SIZE;

  // index of the thread in the warp
  idx.t = _tid_x % WARP_SIZE;

  return idx;
}

__noinline__ void MT_generate_array() {
...
void MT_read_status(indices idx) {
...
void MT_write_status(indices idx) {
  uint MT_idx = idx.b * SIMULATION_WARPS_PER_BLOCK + idx.w;
...
int isWhere(float p, uint nreg, float *bounds) {
...
uint howfar(indices idx, particle_t &p, float &t) {
  if (p.region > 0) {
...
void uphi21(indices idx, float costhe, float sinthe, particle_t &p) {
...
void score_pixel(indices idx, int x, int y, uchar cat, float wt, float e) {
...
void new_particle(indices idx, particle_t &p, volatile float *weight_list) {
  float r1 = get_rand(idx);
  ...
  if (p.process) {
    ...
    float3 target_point = float3(source.rectangle_min.x + source.rectangle_size.x * r1, 
...
void cutoff_discard(indices idx, particle_t &p) {
  if (p.process)
    p.status = p_empty;
}

void user_discard(indices idx, particle_t &p) {
  if (!p.process)
    return;
  ...
  float2 pos = float2(p.x + delta * p.u,
      p.y + delta * p.v);

  // find pixel where the photon hits the detector
  float2 pix = float2((pos.x - detector.center.x) / detector.d.x + (float)detector.N.x / 2.0F,
      (pos.y - detector.center.y) / detector.d.y + (float)detector.N.y / 2.0F);

  float2 lower = float2(floorf(pix.x), floorf(pix.y));
  float2 upper = float2(ceilf(pix.x), ceilf(pix.y));
...
void photon_step(indices idx, particle_t &p) {
...
void rayleigh(indices idx, particle_t &p) {
  ...
  } while (!__all(loop_done));

  sinthe = sqrtf(1.0F - costhe2);
  uphi21(idx, costhe, sinthe, p);
}

void compton(indices idx, particle_t &p) {
  if (p.process) {
    p.status = p_photon_step;
...
void pair_production(indices idx, particle_t &p) {
  if (p.process)
    p.status = p_empty;
}

void photo(indices idx, particle_t &p) {
  if (p.process)
    p.status = p_empty;
}
extern "C"

void simulation_step_kernel(bool init, bool limit_reached) {
    #pragma HLS INTERFACE s_axilite port=init
    #pragma HLS INTERFACE s_axilite port=limit_reached
    #pragma HLS INTERFACE s_axilite port=return
    #pragma HLS ARRAY_PARTITION variable=list_depth_shared complete dim=1
    #pragma HLS ARRAY_PARTITION variable=num_inner_iterations_shared complete dim=1
    #pragma HLS ARRAY_PARTITION variable=step_counters_shared complete dim=1
    #pragma HLS ARRAY_PARTITION variable=combined_weight_list_shared complete dim=1
    #pragma HLS ARRAY_PARTITION variable=weight_list_shared complete dim=1
    ...
    for (int _bid_y = 0; _bid_y < GRID_DIM_Y; _bid_y++) {
        for (int _bid_x = 0; _bid_x < GRID_DIM_X; _bid_x++) {
            ...
            #pragma HLS PIPELINE II=1
                ...
                for (uint j = _tid_x; j < detector.N.x * detector.N.y; j += BLOCK_DIM_X) {
                ...
                if (__any(p.process)) {
                photon_step(idx, p);
                uint count_mask = __ballot(p.process);
                if (idx.t == 0) {
                step_counters[p_photon_step] += __builtin_popcount(count_mask);
                #ifdef DO_LIST_DEPTH_COUNT
                list_depth[idx.w] += 1;
                #endif
                }
                ...
                uint count_mask = __ballot(p.process);
                if (idx.t == 0) {
                step_counters[p_new_particle] += __builtin_popcount(count_mask);
                #ifdef DO_LIST_DEPTH_COUNT
                list_depth[idx.w] += 1;
                #endif
                }
                ...
                uint count_mask = __ballot(p.process);
                if (idx.t == 0) {
                step_counters[p_user_discard] += __builtin_popcount(count_mask);
                #ifdef DO_LIST_DEPTH_COUNT
                list_depth[idx.w] += 1;
                #endif
                }
                ...
                uint count_mask = __ballot(p.process);
                if (idx.t == 0) {
                step_counters[p_compton] += __builtin_popcount(count_mask);
                #ifdef DO_LIST_DEPTH_COUNT
                list_depth[idx.w] += 1;
                #endif
                }
                ...
                uint count_mask = __ballot(p.process);
                if (idx.t == 0) {
                step_counters[p_photo] += __builtin_popcount(count_mask);
                #ifdef DO_LIST_DEPTH_COUNT
                list_depth[idx.w] += 1;
                #endif
                }
                ...
                uint count_mask = __ballot(p.process);
                if (idx.t == 0) {
                step_counters[p_rayleigh] += __builtin_popcount(count_mask);
                #ifdef DO_LIST_DEPTH_COUNT
                list_depth[idx.w] += 1;
                #endif
                }
                ...
                uint count_mask = __ballot(p.process);
                if (idx.t == 0) {
                step_counters[p_pair] += __builtin_popcount(count_mask);
                #ifdef DO_LIST_DEPTH_COUNT
                list_depth[idx.w] += 1;
                #endif
                }
                ...
                uint count_mask = __ballot(p.process);
                if (idx.t == 0) {
                step_counters[p_cutoff_discard] += __builtin_popcount(count_mask);
                #ifdef DO_LIST_DEPTH_COUNT
                list_depth[idx.w] += 1;
                #endif
                }
                ...
                total_count += step_counters_shared[i][_tid_x];

                (*total_step_counts)[idx.b][_tid_x] = total_count;
                }
...
void sum_detector_scores_kernel() {
    #pragma HLS INTERFACE s_axilite port=return
    ...
    for (int _bid_y = 0; _bid_y < GRID_DIM_Y; _bid_y++) {
        for (int _bid_x = 0; _bid_x < GRID_DIM_X; _bid_x++) {
            ...
            #pragma HLS PIPELINE II=1
                ...
                bool do_count = _bid_x / NUM_DETECTOR_CAT;
                uchar cat = _bid_x % NUM_DETECTOR_CAT;
                ...
                for (uint i = _tid_x; i < detector.N.x * detector.N.y; i += BLOCK_DIM_X) {
                double total = 0.0F;
