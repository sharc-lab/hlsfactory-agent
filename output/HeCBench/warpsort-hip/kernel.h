...
sort(const std::vector<float>& data, double &time) {
  ...
  float* devFloat = NULL;
  hipMalloc(&devFloat, sizeBytes);
  hipMemcpy(devFloat, data.data(), sizeBytes, hipMemcpyHostToDevice);

  float* devResult = NULL;
  hipMalloc(&devResult, sizeBytes);
  hipMemset(devResult, 0, sizeBytes);
...
sortWithIndices(const std::vector<float>& data, double &time) {
  ...
  hipMemset(devResult, 0, sizeBytes);
  int* devIndices = NULL;
  hipMalloc(&devIndices, sizeIndicesBytes);
  hipMemset(devIndices, 0, sizeIndicesBytes);
  ...
  std::vector<float> vals(data.size());
  hipMemcpy(vals.data(),
             devResult, sizeBytes, hipMemcpyDeviceToHost);

  std::vector<int> indices(data.size());
  hipMemcpy(indices.data(),
             devIndices, sizeIndicesBytes, hipMemcpyDeviceToHost);
