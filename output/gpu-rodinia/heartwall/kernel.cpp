...
void kernel(){
    ...
    #pragma HLS ARRAY_PARTITION variable=in_partial_sum complete dim=1
    #pragma HLS ARRAY_PARTITION variable=in_sqr_partial_sum complete dim=1
    #pragma HLS ARRAY_PARTITION variable=par_max_val complete dim=1
    #pragma HLS ARRAY_PARTITION variable=par_max_coo complete dim=1
    #pragma HLS ARRAY_PARTITION variable=d_in_mod_temp complete dim=1
    ...
    for (int _bid_x = 0; _bid_x < GRID_DIM_X; _bid_x++) {
        ...
        #pragma HLS PIPELINE II=1
            ...
            int bx = _bid_x;																// get current horizontal block index (0-n)
            int tx = _tid_x;																// get current horizontal thread index (0-n)
            int ei_new;
