...
void SemiGlobalLinearShortLong::InitializeScores(
...
void SemiGlobalLinearShortLong::PE::Compute(char_t local_query_val,
...
void SemiGlobalLinearShortLong::UpdatePEMaximum(
...
#pragma HLS unroll
        if (predicate[i])
        {
            if (scores[i + 1][LAYER_MAXIMIUM] > max[i].score)
            {
                ...
                if (i + chunk_row_offset == query_len - 1 )
...
void SemiGlobalLinearShortLong::InitializeMaxScores(ScorePack (&max)[PE_NUM], idx_t qry_len, idx_t ref_len){
    for (int i = 0; i < PE_NUM; i++)
    {
#pragma HLS unroll
...
void SemiGlobalLinearShortLong::Traceback::StateInit(tbp_t tbp, TB_STATE &state){
    if (tbp == TB_DIAG)
    {
        state = TB_STATE::MM;
    }
...
void SemiGlobalLinearShortLong::Traceback::StateMapping(tbp_t tbp, TB_STATE &state, tbr_t &navigation){
